...
static fd_t *fd_list;
static int fd_max, sig_count;
static fd_set fd_read, fd_write, fd_excep;
static sig_table_t **signal_table;
...
int signal_register(sig_t signal, sig_func_t func, void *data)
{
	...
	if (signal >= sig_count || !(s = alloc_sig_table_t(signal, func, data)))
		return -1;
...
int event_init(int sc)
{
	...
	if (sc && !(signal_table = (sig_table_t**)calloc(sc,
		sizeof(sig_table_t*)))) {
		return -1;
	}
